...
2. `supabase/migrations/` 안의 파일을 번호 순서대로 SQL Editor에서 실행합니다
   (`0001_init.sql` → `0002_functions.sql` → `0003_channel_stats.sql` → `0004_watched.sql`).
...
목록은 한 페이지에 **60개**씩 보여주고 아래에 페이지 번호가 붙습니다(`?page=`).
페이지가 많으면 `1 … 6 7 8 … 13`처럼 현재 위치 주변만 펼쳐집니다.
폴더에 적힌 숫자는 전체 개수이므로 목록 위의 `전체 N개 중 x–y번째`와 비교해 보세요.

영상을 클릭해 유튜브로 나가면 **본 영상**으로 기록되고(`videos.watched_at`), 다음에
목록을 열 때 맨 아래로 내려갑니다. 카드는 회색조로 바뀌고 `✓ 본 영상` 뱃지가 붙습니다.
잘못 눌렀다면 그 뱃지를 다시 눌러 해제할 수 있습니다. 목록이 클릭하는 순간 재정렬되면
오히려 불편하므로, 순서는 다음에 열 때 반영됩니다.
...
이 값이 1000을 넘어도 되도록 조회를 페이지 단위로 나눠서 가져옵니다 — PostgREST가
한 응답에 1000행까지만 돌려주기 때문에, 예전에는 1000을 넘겨 요청해도 조용히 잘렸습니다.
(실측: 영상 6,446개 갱신에 49초. 워크플로는 상한 8000, 예산 600초로 돌립니다.)
